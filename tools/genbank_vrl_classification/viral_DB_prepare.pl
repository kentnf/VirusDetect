...
'; 
	...
	# cd-hit
	my $cdhit_bin = resolve_external_tool("cd-hit-est");
...
sub resolve_external_tool
{
	my $tool_name = shift;

	if (defined $ENV{'VIRUSDETECT_TOOL_DIR'} && $ENV{'VIRUSDETECT_TOOL_DIR'} ne '') {
		my $candidate = $ENV{'VIRUSDETECT_TOOL_DIR'}."/".$tool_name;
		return $candidate if -x $candidate;
	}

	foreach my $path_dir (split(/:/, $ENV{'PATH'} || '')) {
		next unless defined $path_dir && $path_dir ne '';
		my $candidate = $path_dir."/".$tool_name;
		return $candidate if -x $candidate;
	}

	die "[ERR]could not find required tool '$tool_name'; set VIRUSDETECT_TOOL_DIR or add it to PATH\n";
}
...
';
	...
	my $samtools_bin = resolve_external_tool("samtools");
