...
```
$ bwa index vrl_Plants_all_U95.fasta
$ samtools faidx vrl_Plants_all_U95.fasta
$ formatdb -i vrl_Plants_all_U95.fasta -p F
$ formatdb -i vrl_Plants_all_U95.fasta_prot
```
If these tools are not already on `PATH`, set `VIRUSDETECT_TOOL_DIR` to a directory that contains them.
